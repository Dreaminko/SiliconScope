...
> 이 문서는 ktop 코드베이스에서 작업하는 Claude를 위한 지침이다. 결정사항/규약은 여기서 확정한다.
...
├──────────────────────────────────────────┤
│ KtopCore (데이터 라이브러리, UI 비의존)      │  ← 두 UI가 공유
...
**원칙**: KtopCore는 SwiftUI에 의존하지 않는다. 메뉴바·전체창·CLI가 모두 같은 KtopCore를 링크한다.
...
ktop/
├── CLAUDE.md
├── Package.swift            # SPM: KtopCore 라이브러리 + 검증 CLI
├── Sources/
│   ├── CIOReport/           # IOReport extern 선언 (C 타깃 또는 dlopen 래퍼)
│   ├── KtopCore/            # 데이터 레이어 (UI 비의존)
│   └── ktop-cli/            # 지표 터미널 출력 (검증용)
├── App/                     # SwiftUI 앱 (2단계, Xcode 프로젝트)
...
```bash
xcrun swift build                  # 전체 빌드 (CIOReport + KtopCore + CLI + app)
xcrun swift run -q ktop-cli        # 지표 출력으로 데이터 레이어 검증 (sudo 불필요)
xcrun swift run WhisPlayInfo       # SwiftUI GUI (대시보드 창 + 메뉴바). sudo 불필요
xcrun swift test                   # KtopCore 단위 테스트
```

GUI는 현재 SPM 실행 타깃(`Sources/WhisPlayInfo`, SwiftUI `@main`)으로 개발용 실행한다.
(앱 표시명·실행파일·프로세스명 모두 **WhisPlayInfo**. WhisPlay 앱의 ANE/Media 사용 추적용 동반 도구.)
...
- KtopCore에 **SwiftUI import 금지** (레이어 분리 유지).
